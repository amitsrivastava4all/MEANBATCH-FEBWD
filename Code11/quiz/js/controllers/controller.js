window.addEventListener("load",function(){
	...
	document.getElementById("saveBt").addEventListener("click",saveData);
	
	document.getElementById("loadBt").addEventListener("click",loadData);
	
	printQid();
});

function saveData(){
	var jsonString = JSON.stringify(questionOperations.questionArray);
	if(localStorage){
		localStorage.mydata  = jsonString;
		alert("Data Saved...");
	}
	else{
		alert("U Don't Have Updated Browser....");
	}
}

function loadData(){
	if(localStorage && localStorage.mydata){
		questionOperations.questionArray = JSON.parse(localStorage.mydata);
		printTable(questionOperations.questionArray);
	}
	else{
		alert("Either Browser is Old or Data is not saved...");
	}
}

function sortById(){
	var questionArray = questionOperations.sortById();
